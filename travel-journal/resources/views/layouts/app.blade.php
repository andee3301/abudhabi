...
<body class="font-sans antialiased min-h-screen bg-slate-100 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
    <div class="backdrop-blur flex min-h-screen flex-col">
        <nav class="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl dark:border-slate-800/70 dark:bg-slate-900/80">
            <div class="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                <div class="flex items-center gap-8">
                    <a href="{{ route('dashboard') }}" class="text-lg font-semibold text-emerald-700 dark:text-emerald-300">Treep</a>
                    <div class="hidden md:flex items-center gap-5 text-sm text-slate-700 dark:text-slate-200">
                        <a href="{{ route('dashboard') }}" class="hover:text-emerald-600 dark:hover:text-emerald-300">Dashboard</a>
                        <a href="{{ route('trips.index') }}" class="hover:text-emerald-600 dark:hover:text-emerald-300">Journeys</a>
                        <a href="{{ route('explore.index') }}" class="hover:text-emerald-600 dark:hover:text-emerald-300">Intel</a>
                        <a href="{{ route('journal.create') }}" class="hover:text-emerald-600 dark:hover:text-emerald-300">New note</a>
                    </div>
                </div>
                <div class="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-200">
                    <button type="button" data-theme-toggle aria-label="Toggle theme"
                        class="flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-lg transition hover:border-emerald-400 hover:text-emerald-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100">
                        🌙
                    </button>
                    @auth
